<?php
class mysqlthiago extends mysqli
{
    public $debug = false;
    public $nopwd = false;
    public $errno_thiago = 0;
    public $error_mysqlthiago = '';
    public function __construct($host = null, $user = null, $pass = null, $db = null)
    {
        ...
        if (empty($host) && null !== HOST && null !== USER && null !== PASS && null !== DB) {
            ...
            if ($host === null) {
                ...
                $this->error_mysqlthiago = 'Você deve informar um HOST;';
                return array();
            }
            if ($user === null) {
                error_reporting(E_ALL);
                $this->error_mysqlthiago = 'Você deve informar um USUÁRIO;';
                return array();
            }
            if ($pass === null) {
                error_reporting(E_ALL);
                $this->error_mysqlthiago = 'Você deve informar uma SENHA;';
                return array();
            }
            if ($db === null) {
                error_reporting(E_ALL);
                $this->error_mysqlthiago = 'Você deve informar um BANCO DE DADOS;';
                return array();
            }
        }
        ...
        if (!$check) {
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'O servidor não responde nesse endereço e porta;';
            exit;
        }
    ...
    protected function verifica_erros($tabela = null, $dados = array())
    {
        ...
        if (empty($tabela)) {
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'Você deve informar um nome de TABELA como primeiro parâmetro. Sintaxe insert( tabela,dados(array) );';
            $this->errno=1;
            return true;
        }
        if (!is_string($tabela)) {
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'Você deve informar uma variavel string como primeiro parâmetro. Sintaxe insert( tabela(string),dados(array) );';
            $this->errno=1;
            return true;
        }
        if ($dados != '_' && (empty($dados) || !is_array($dados))) {
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'Você deve informar um array multidimensional como segundo parâmetro. Sintaxe insert( tabela(string),dados(array) );';
            $this->errno=1;
            return true;
        }
        ...
        if ($result = $this->query('SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_NAME="' . $tabela . '"')) {
            ...
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'Não foi possível selecionar na tabela INFORMATION_SCHEMA.TABLES;';
            $this->errno=1;
            return true;
        }
        if (empty($resultset)) {
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'Você deve informar uma tabela que exista;';
            $this->errno=1;
            return true;
        }
        return false;
    ...
    public function insert($tabela = null, $dados = array(), $permite_funcoes = array(), $debug = false, $roda = true){
        ...
        if ($result = $this->query('SHOW COLUMNS FROM ' . $tabela)) {
            ...
            error_reporting(E_ALL);
            $this->error_mysqlthiago = 'Não foi possível executar SHOW COLUMNS;';
            if ($this->debug){
            	echo $this->error_mysqlthiago.'<br>';
            	exit;
            }
            $this->errno_thiago = 1;
            return array();
        ...
        foreach (array_keys($dados) as $campo) {
            if (!isset($campos_por_nome[$campo])) {
                error_reporting(E_ALL);
                $this->error_mysqlthiago = "O campo $campo não existe na tabela.";
                if ($this->debug){
                	echo $this->error_mysqlthiago.'<br>';
                	exit;
                }
        ...
        foreach ($notnull as $campo) {
            if (!isset($dados[$campo])){
                error_reporting(E_ALL);
                $this->error_mysqlthiago = "Você obrigatoriamente deve fornecer o campo $campo;";
                if ($debug){
                	echo $this->error_mysqlthiago.'<br>';
                	exit;
                }
        ...
        if ($stmt = mysqli_prepare($this, $sql)) {
            ...
            foreach ($params as $queryKey => $query) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    $queryResult[] = false;
                    if (!empty($this->error)) $this->error_mysqlthiago=$this->error;
                }
